...
class WindowsBot {
    ...
    */
    ...
    //新增监听开关方法
    static setSwitchListener(projectRoot, keyName) {
        const listener = new GlobalKeyboardListener();
        listener.addListener(async function (e) {
            if (e.state === "UP" && e.name === keyName) {
                fs.writeFileSync(path.join(WindowsBot.rootpath, 'stop.flag'), '');
            }
        })
        WindowsBot.rootpath = projectRoot;
        WindowsBot.keylistener = listener;
    }

    // 新增工厂方法（关键添加）
    static async create(ip, port) {
        // 端口检查逻辑
        const isPortInUse = await WindowsBot.checkPort(port);
        let pid = null;
        if (isPortInUse) {
            console.log(`端口 ${port} 被占用，尝试关闭服务...`);
        }
        // 驱动启动逻辑
        ...
        if (ip == "127.0.0.1") {
            ...
            pid = child_process.spawn(driverPath, [ip, port]);
            console.log(`正在启动WindowsDriver... PID: ${pid.pid}`);
        }
        ...
        return new Promise((resolve, reject) => {
            ...
            // 设置10秒超时，如果没有收到连接则关闭驱动
            timeoutId = setTimeout(() => {
                console.log(`10秒内未收到连接，关闭WindowsDriver进程 ${pid.pid}`);
                if (pid && !pid.killed) {
                    pid.kill();  // 关闭驱动进程
                }
                server.close(() => {
                ...
                reject(new Error(`10秒内未建立连接，WindowsDriver进程已关闭`));
            }, 10000); // 10秒超时
            ...
            server.listen(port, () => {
                server.on('connection', (clientSocket) => {
                    ...
                    const instance = new WindowsBot(clientSocket);
                    instance.server = server;  // 绑定专属server
                    instance.pid = pid; // 绑定进程ID
                    console.log(`服务端已启动在端口 ${port}，收到连接`);
                    resolve(instance);
                });
                ...
                server.on('error', (err) => {
                    ...
                    if (!connectionReceived && timeoutId) {
                        clearTimeout(timeoutId);
                        timeoutId = null;
                    }
                    if (pid && !pid.killed) {
            ...
            if (pid) {
                pid.on('error', (err) => {
                    console.error(`WindowsDriver进程错误: ${err.message}`);
                    if (timeoutId) {
                        clearTimeout(timeoutId);
                        timeoutId = null;
                    }
                ...
                pid.on('close', (code) => {
                    console.log(`WindowsDriver进程 ${pid.pid} 已退出，退出码: ${code}`);
                    if (!connectionReceived && timeoutId) {
                        clearTimeout(timeoutId);
                        timeoutId = null;
                    }
    ...
    constructor(clientSocket) {
        ...
        this.socket.on('error', error => {
            error?.code === 'ECONNRESET' ? '' : console.log('Socket连接异常:', error)
            this.socket.destroy(); // 主动销毁socket
            this.mutex.release();  // 释放可能被锁定的资源
            this.resolveHand = null; // 清空未完成的请求
        });
        ...
        this.socket.on('data', data => {
            if (!this.socket.writable) { // 新增连接状态检查
                console.error('尝试在已关闭的连接上接收数据');
                return;
            }
    ...
    */
    sendData = (strData) => {
        // 新增终止检查逻辑
        const STOP_FLAG_PATH = path.join(WindowsBot.rootpath ? WindowsBot.rootpath : '', 'stop.flag');
        if (fs.existsSync(STOP_FLAG_PATH)) {
            fs.unlinkSync(STOP_FLAG_PATH);
            console.log('[SYSTEM] RPA任务已被强制终止');
            return;
    ...
    */
     ...
     * @return {Promise.<boolean>} 成功返回true，失败返回false
    */
    async initMetahuman(metahumanModePath, metahumanScaleWidth, metahumanScaleHeight, isUpdateMetahuman = false) {
        let strData = this.setSendData("initMetahuman", metahumanModePath, metahumanScaleWidth, metahumanScaleHeight, isUpdateMetahuman);
        let byteRet = await this.sendData(strData);
        let strRet = byteRet.toString();
        if (strRet == "false")
            return false;
    ...
    */
    async close() {
        try {
            ...
            // 清理全局键盘监听器
            if (WindowsBot.keylistener) {
                WindowsBot.keylistener.removeAllListeners();
                WindowsBot.keylistener = null;
            }
            ...
            console.log('WindowsBot资源已成功清理');
            return true;
        } catch (error) {
            console.error('清理WindowsBot资源时出错:', error);
            return false;
        }
    }
